...
- The soft-skill embedding threshold (currently 0.48 default, see `passthebot.normalizer.extract_soft_skills`) was raised from an initial 0.45 placeholder on 2026-09-14 after a real-CV smoke test with a fuller soft-skill graph (6 entries) surfaced two borderline false positives (problem_solving/analytical_thinking scoring 0.458/0.472 against an unrelated sentence fragment) sitting right next to a genuine match at 0.490 (communication). 0.48 was chosen empirically to sit between those two clusters for this one test case. This is still not a validated value from a proper calibration harness — it is now evidence-based from n=1 real resume instead of the original n=1 synthetic example, which is progress but not the multi-phrase, multi-document harness (a labeled set of matching/non-matching sentences per soft-skill entry, scored to pick a threshold that balances false positives/negatives) this note originally called for. That harness should still be built once more real postings/resumes are run through the engine.
